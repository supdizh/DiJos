...
trap_init(void)
{
	...
	SETGATE (idt[IRQ_OFFSET + 0], 0, GD_KT, handler_irq0, 0);//pri = 0,dont check for IRQ
...
trap_init_percpu(void)
{
	...
	int cur_cpu_idx = thiscpu->cpu_id;
	thiscpu->cpu_ts.ts_esp0 = KSTACKTOP - cur_cpu_idx*(KSTKSIZE + KSTKGAP);
	thiscpu->cpu_ts.ts_ss0 = GD_KD;

	// Initialize the TSS slot of the gdt.
	gdt[(GD_TSS0 >> 3) + cur_cpu_idx] = SEG16(STS_T32A, (uint32_t)&(thiscpu->cpu_ts),
					sizeof(struct Taskstate), 0); 	
	gdt[(GD_TSS0 >> 3) + cur_cpu_idx].sd_s = 0;

	// Load the TSS selector (like other segment selectors, the
	// bottom three bits are special; we leave them 0)
	ltr((GD_TSS0 + (cur_cpu_idx << 3)) & ~0x7);
...
trap_dispatch(struct Trapframe *tf)
{
	...
	switch(tf->tf_trapno){
		...
		case T_SYSCALL:
			...
			//allowing syscall to return negative, for ipc usage
...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 3: Your code here.
	if((tf->tf_cs & 3) == 0){
		cprintf("fault_va = 0x%08x, ip = 0x%08x\n",fault_va, tf->tf_eip);
		panic("kern/trap.c/page_fault_handler: page fault from kernel\n");
	}
	...
	if(curenv->env_pgfault_upcall != NULL){
		...
		env_run(curenv);//to the handler. handler to program, i dont know
		//this call do not return
	}
