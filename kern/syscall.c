...
sys_exofork(void)
{
	...
	//copy the trapframe so that the child will trap return to the same place
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	//now get the pages of source side
	pte_t *pte;
	struct PageInfo *pp;
	pp = page_lookup(srcenv->env_pgdir, srcva, &pte);
	if(pp == NULL)//cant find it
		return -E_INVAL;
	if(!(*pte & PTE_W) && (perm & PTE_W))//its a read-only page! cannot change to WR
		return -E_INVAL;
...
{
	// LAB 4: Your code here.
	//cprintf("into ipc_try_send to %d\n",envid);
	struct Env *dstenv;
	int r;
	//	-E_BAD_ENV if environment envid doesn't currently exist.
	//		(No need to check permissions.)
	...
	dstenv->env_tf.tf_regs.reg_eax = 0;//directly return from trap with 0
	return 0;
}
...
// return 0 on success(directly run_env return from trapframe)
...
sys_ipc_recv(void *dstva)
{
	...
	curenv->env_ipc_from = 0;//not yet any
	//ipc only valid between 2 process

	curenv->env_status = ENV_NOT_RUNNABLE;
	cprintf("%d yielding and waiting for recv\n",curenv->env_id);
	sched_yield();
	return 0;
}
